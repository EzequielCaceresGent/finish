...
async function post(req, res) {
    ...
    const {name, description, budget, clientContact, client} = req.body;
    ...
    const proposal = {
        name, description, deadline, budget, clientContact: contact.dni, client
    };
    ...
    try {
        ...
        await db.execute(`INSERT INTO Propuesta(nombrePropuesta, descripcionNecesidadCliente, restriccionTemporal, restriccionEconomica, FK_referenteDelCliente, FK_empresaCliente, FK_empleadoComercialAsociado, directorioArchivosPropuesta) VALUES (?, ?, ?, ?, ?, ?, ?, ?);`, [...Object.values(proposal), req.employee.PK_FK_dniEmpleado, directory]);
    } catch (error) {
        console.log(error);
    ...
    upload(req, res, async error => {
        ...
        try {
            ...
            if (req.file !== undefined) {
                ...
                await db.execute("UPDATE Propuesta SET progreso = 'Enviar al cliente', rutaPropuestaTecnica = ?, FK_empleadoTecnicoAsociado = ? WHERE Propuesta.PK_idPropuesta = ?", [req.file.path, req.employee.PK_FK_dniEmpleado, req.params.proposalId]);   
            }
